program ising1
    ...
    subroutine metropolis(T)
        ...
        if (archivo_append == 1) then
            open(unit=14, file="energia2.dat", status="unknown", position="append")
        else if (archivo_append == 0) then
            open(unit=14, file="energia2.dat", status="unknown")
        end if
    ...
    write(14,*) T, promedio, -0.5*tanh(0.5/T), promedio_M, tanh(0.5/T)
